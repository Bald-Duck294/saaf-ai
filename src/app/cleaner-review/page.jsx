...
import { ListChecks, Clock, CheckCircle, Calendar, MapPin, RotateCcw } from "lucide-react";
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6">
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-3xl font-bold text-slate-800">Cleaner Activity</h1>
              </div>
            ...
            <div className="mt-4 flex flex-col md:flex-row items-center gap-4 p-4 bg-white rounded-lg shadow-sm border">
               <div className="w-full md:w-auto">
                  <label htmlFor="date-filter" className="text-sm font-semibold text-slate-600 mr-2">Filter by Date:</label>
                  <input 
                    type="date" 
                    id="date-filter"
                    value={date}
               ...
               <button onClick={handleReset} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-md transition">
                  <RotateCcw size={14} />
                  Reset Filters
               </button>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {isLoading ? (
              ...
              reviews.map((review) => (
                <div key={review.id} className="bg-white p-5 rounded-lg shadow-sm border border-slate-200 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start">
                      <h3 className="text-lg font-bold text-slate-800 pr-2">{review.name}</h3>
                      ...
                      >
                        {review.status === 'completed' ? <CheckCircle size={14} /> : <Clock size={14} />}
                        {review.status}
                      </span>
                    </div>
              ...
              <div className="col-span-full text-center py-16 bg-white rounded-lg shadow-sm">
                <h3 className="text-xl font-semibold text-slate-700">No Reviews Found</h3>
                <p className="text-slate-500 mt-2">There are no tasks matching the current filter.</p>
              </div>
            )}
          </div>
        </div>
