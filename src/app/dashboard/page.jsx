...
export default function Dashboard() {
  ...
  const [statsData, setStatsData] = useState({
    totalLocations: 0,
    totalUsers: 128, // Using a placeholder as requested
    avgRating: "0.0", // Will be calculated if possible, otherwise default
    ongoingTasks: 0,
    completedTasks: 0,
  });
  const [topLocations, setTopLocations] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const router = useRouter(); // Initialize the rout
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Process locations data for stats and top locations
        if (locationsRes.success) {
          setStatsData(prev => ({ ...prev, totalLocations: locationsRes.data.length }));
        ...
        if (ongoingRes.success) {
          setStatsData(prev => ({ ...prev, ongoingTasks: ongoingRes.data.length }));
        }
        if (completedRes.success) {
          setStatsData(prev => ({ ...prev, completedTasks: completedRes.data.length }));
        }
  ...
  const chartData = [ /* Static for now */
    { org: "Org A", locations: 10 }, { org: "Org B", locations: 15 },
    { org: "Org C", locations: 7 }, { org: "Org D", locations: 12 },
  ];

  const recentActivities = [ /* Static for now */
  ...
  const activityColors = {
    success: "bg-emerald-500", info: "bg-sky-500",
    update: "bg-violet-500", warning: "bg-amber-500",
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200/60 lg:col-span-1">
            <h2 className="font-semibold text-lg text-slate-800 mb-4">Top Rated Locations</h2>
            {isLoading ? (
          ...
          {/* Right: Bar Chart (Static for now) */}
          <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200/60 lg:col-span-2">
            <h2 className="font-semibold text-lg text-slate-800 mb-4">Locations by Organization</h2>
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                <XAxis dataKey="org" tick={{ fill: "#64748b", fontSize: 12 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: "#64748b", fontSize: 12 }} axisLine={false} tickLine={false} />
                <Tooltip contentStyle={{ background: 'rgba(255, 255, 255, 0.8)', backdropFilter: 'blur(5px)', border: '1px solid #e2e8f0', borderRadius: '0.75rem' }} />
        ...
        {/* Bottom: Recent Activities (Static for now) */}
        <div className="bg-white rounded-xl shadow-sm p-6 mt-8 border border-slate-200/60">
          ...
          <ul className="space-y-2">
            {recentActivities.map((activity, idx) => (
              <li key={idx} className="flex items-center gap-4 p-3 rounded-lg hover:bg-slate-50 transition-colors">
